    ...
    var VibePricingAdmin = {
        ...
        initRuleForm: function() {
            ...
            $('#rule_name').on('blur', function() {
                ...
                if (name.length < 3) {
                    $(this).addClass('error');
                    $(this).after('<span class="error-message">Rule name must be at least 3 characters long.</span>');
                } else {
            ...
            $('#adjustment_value').on('input', function() {
                ...
                if (type === 'percentage' && (value < -100 || value > 1000)) {
                    $(this).addClass('error');
                    $(this).siblings('.error-message').remove();
                    $(this).after('<span class="error-message">Percentage should be between -100% and 1000%.</span>');
                } else if (type === 'fixed_price' && value < 0) {
                    $(this).addClass('error');
                    $(this).siblings('.error-message').remove();
                    $(this).after('<span class="error-message">Fixed price cannot be negative.</span>');
                } else {
                    $(this).removeClass('error');
                    $(this).siblings('.error-message').remove();
                }
        ...
        initProductSelectionHelpers: function() {
            ...
            $('.target-option select[multiple]').each(function() {
                ...
                if ($container.find('.select-helpers').length === 0) {
                    var $helpers = $('<div class="select-helpers" style="margin-top: 5px;"></div>');
                    $helpers.append('<button type="button" class="button select-all">Select All</button> ');
                    $helpers.append('<button type="button" class="button select-none">Select None</button>');
            ...
            $('input[name="min_price"], input[name="max_price"]').on('input', function() {
                ...
                if (minPrice > maxPrice && maxPrice !== Infinity) {
                    $(this).addClass('error');
                    $(this).siblings('.error-message').remove();
                    $(this).after('<span class="error-message">Minimum price cannot be greater than maximum price.</span>');
                } else {
                    $('input[name="min_price"], input[name="max_price"]').removeClass('error');
                    $('input[name="min_price"], input[name="max_price"]').siblings('.error-message').remove();
                }
            });
        },
        ...
        initComplexLogicHelper: function() {
            ...
            if ($complexTextarea.length > 0) {
                var $helpButton = $('<button type="button" class="button" style="margin-top: 5px;">Show Syntax Help</button>');
                var $helpDiv = $('<div class="complex-logic-help" style="display: none; margin-top: 10px; padding: 10px; background: #f0f8ff; border: 1px solid #b3d9ff; border-radius: 3px;"></div>');
                ...
                $helpButton.click(function() {
                    $helpDiv.toggle();
                    $(this).text($helpDiv.is(':visible') ? 'Hide Syntax Help' : 'Show Syntax Help');
                });
            }
        },
        ...
        initBulkActions: function() {
            ...
            $('#doaction').on('click', function(e) {
                ...
                if (action === 'delete') {
                    var confirmMessage = 'Are you sure you want to delete ' + selectedRules.length + ' rule(s)?';
        ...
        initAjaxHandlers: function() {
            ...
            $('.clear-cache-btn').on('click', function(e) {
                ...
                .done(function(response) {
                    if (response.success) {
                        $button.text('Cleared!').removeClass('button').addClass('button-primary');
                        setTimeout(function() {
        ...
        initConfirmActions: function() {
            ...
            $('.delete-rule').on('click', function(e) {
                ...
                var confirmMessage = 'Are you sure you want to delete the rule "' + ruleName + '"?';
            ...
            $('input[name="emergency_disable"]').on('change', function() {
                if ($(this).prop('checked')) {
                    var confirmed = confirm('This will immediately disable ALL dynamic pricing functionality. Are you sure?');
                    if (!confirmed) {
        ...
        updatePerformanceStats: function(stats) {
            ...
            if (stats.object_cache_enabled !== undefined) {
                $container.find('.object-cache-status').text(stats.object_cache_enabled ? 'Yes' : 'No');
            }
            ...
            if (stats.database_cache_size !== undefined) {
                $container.find('.db-cache-size').text(this.formatBytes(stats.database_cache_size));
            }
