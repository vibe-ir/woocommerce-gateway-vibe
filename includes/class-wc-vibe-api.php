...
class WC_Vibe_API {
	...
	public function get_products( $request ) {
		...
		foreach ( $products as $product ) {
			$response[] = array(
				...
				'product-available' => $product->is_in_stock(),
			);
		}
	...
	public function get_product_details( $request ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			foreach ( $variations as $variation ) {
				...
				$variants[] = array(
					...
					'price-main'       => $variation_obj->get_regular_price(),
					'price-sale'       => $variation_obj->get_sale_price() ? $variation_obj->get_sale_price() : $variation_obj->get_regular_price(),
		...
		$response = array(
			...
			'price-main'          => $product->get_regular_price(),
			'price-sale'          => $product->get_sale_price() ? $product->get_sale_price() : $product->get_regular_price(),
